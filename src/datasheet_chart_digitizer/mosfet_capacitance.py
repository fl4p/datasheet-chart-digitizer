...
def extract_vector_trace_components(
    ...
    candidates = sorted(candidates, key=lambda pts: _path_length(pts), reverse=True)[:3]
    span_fractions = [
        (max(p[0] for p in component) - min(p[0] for p in component)) / max(1e-9, plot_rect.width)
    ...
    ordered = sorted(candidates, key=_right_edge_y)
    names = ["Ciss", "Coss", "Crss"]
    traces: list[Trace] = []
    for name, pts_pdf in zip(names, ordered):
        points = [
            (
        ...
        points = _dedupe_adjacent_points(points)
        xs = [p[0] for p in points]
        ys = [p[1] for p in points]
...
def _is_long_orthogonal_segment(p0: tuple[float, float], p1: tuple[float, float], plot_rect) -> bool:
    dx = abs(p1[0] - p0[0])
    dy = abs(p1[1] - p0[1])
    return (dy < 0.1 and dx > plot_rect.width * 0.45) or (dx < 0.1 and dy > plot_rect.height * 0.45)
...
def _order_edge_component(edges: list[VectorEdge], tol: float) -> list[tuple[float, float]]:
    ...
    for idx, edge in enumerate(edges):
        if idx not in used:
            if out and _distance(out[-1], edge.p0) <= _distance(out[-1], edge.p1):
                out.extend(edge.points[1:])
            else:
                out.extend(reversed(edge.points))
    return out
...
def _mostly_inside_plot(points: list[tuple[float, float]], plot_rect) -> bool:
    inside = sum(1 for x, y in points if plot_rect.contains((x, y)))
