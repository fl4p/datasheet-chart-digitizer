...
def find_closed_frame_plot_box(gray: np.ndarray) -> PlotBox:
    ...
    if (
        ...
        return recovered
    open_extent = _open_grid_extent(gray, detected)
...
def _open_grid_extent(gray: np.ndarray, detected: PlotBox) -> PlotBox | None:
    """Recover occluded side rails from repeated owned grid-row endpoints.

    Inline curve labels can merge side gridlines into wide contours, so the
    generic detector stops at an interior line. Require a dominant common row
    endpoint, top/bottom closure, and a regular owned vertical grid before
    extending either side; text is not evidence. The unaffected edges stay on
    the established detector so an imprecise morphology contour cannot move a
    previously trusted axis boundary.
    """

    _height, width = gray.shape
    _, bw = cv2.threshold(gray, 245, 255, cv2.THRESH_BINARY_INV)
    horizontals = _orthogonal_line_boxes(bw, vertical=False)
    edge_tolerance = max(6.0, width * 0.015)
    row_tolerance = max(5.0, detected.height * 0.02)
    rows = [
        (y + (box_height - 1) / 2, float(x), float(x + box_width - 1))
        for x, y, box_width, box_height in horizontals
        if detected.y0 - row_tolerance
        <= y + (box_height - 1) / 2
        <= detected.y1 + row_tolerance
        and x <= detected.x0 + edge_tolerance
        and x + box_width - 1 >= detected.x1 - edge_tolerance
    ]
    if len(rows) < 5:
        return None
    ...
    centers = sorted(
        ...
        if detected.x0 - edge_tolerance <= x <= detected.x1 + edge_tolerance
    ...
    maximum_extension = 0.30 * detected.width
    minimum_support = max(5, int(np.ceil(0.60 * len(rows))))

    def supported_edge(*, left: bool) -> float | None:
        boundary_center = centers[0] if left else centers[-1]
        boundary = detected.x0 if left else detected.x1
        if abs(boundary_center - boundary) > max(5.0, 0.20 * pitch):
            return None
        endpoint_index = 1 if left else 2
        candidates = [
            row[endpoint_index]
            for row in rows
            if (
                detected.x0 - row[endpoint_index]
                if left
                else row[endpoint_index] - detected.x1
            )
            >= minimum_extension
        ]
        groups = [
            [
                row
                for row in rows
                if abs(row[endpoint_index] - candidate) <= edge_tolerance
            ]
            for candidate in candidates
        ]
        if not groups:
            return None
        support = max(groups, key=len)
        if len(support) < minimum_support:
            return None
        support_rows = [row[0] for row in support]
        if (
            not any(
                abs(center_y - detected.y0) <= row_tolerance
                for center_y in support_rows
            )
            or not any(
                abs(center_y - detected.y1) <= row_tolerance
                for center_y in support_rows
            )
        ):
            return None
        edge = float(np.median([row[endpoint_index] for row in support]))
        extension = detected.x0 - edge if left else edge - detected.x1
        if not minimum_extension <= extension <= maximum_extension:
            return None
        if left and edge < width * 0.015:
            return None
        if not left and edge > width * 0.985:
            return None
        return edge

    left = supported_edge(left=True)
    right = supported_edge(left=False)
    if left is None and right is None:
        return None
    return PlotBox(
        int(round(left)) if left is not None else detected.x0,
        detected.y0,
        int(round(right)) if right is not None else detected.x1,
        detected.y1,
    )
