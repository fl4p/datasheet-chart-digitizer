...
class OpenSidePlotBoxTests(unittest.TestCase):
    ...
    def test_common_grid_rows_extend_both_inset_sides(self) -> None:
        gray = np.full((500, 760), 255, dtype=np.uint8)
        # Labels occlude both true side rails, leaving only the interior grid
        # columns as morphology candidates. Every owned horizontal row still
        # proves the same farther endpoints, including the top and bottom.
        for x in (180, 220, 260, 300, 340, 380, 420, 460, 500, 540, 580):
            cv2.line(gray, (x, 55), (x, 420), 0, 2)
        for y in (55, 130, 225, 320, 420):
            cv2.line(gray, (120, y), (620, y), 0, 2)

        detected = find_plot_box(gray)
        self.assertEqual((180, 580), (detected.x0, detected.x1))

        recovered = find_capacitance_plot_box(gray)

        self.assertEqual(
            (120, detected.y0, detected.y1),
            (recovered.x0, recovered.y0, recovered.y1),
        )
        self.assertGreaterEqual(recovered.x1, 620)
        self.assertLessEqual(recovered.x1, 622)

    def test_owned_rows_can_prove_quarter_width_right_extension(self) -> None:
        gray = np.full((500, 700), 255, dtype=np.uint8)
        ...
        detected = find_plot_box(gray)
        self.assertEqual(520, detected.x1)
    ...
    def test_common_rows_ending_at_crop_border_do_not_extend(self) -> None:
        gray = np.full((500, 700), 255, dtype=np.uint8)
        for x in (120, 220, 300, 380, 460, 540, 620):
            cv2.line(gray, (x, 55), (x, 420), 0, 2)
        for y in (55, 130, 225, 320, 420):
            cv2.line(gray, (120, y), (699, y), 0, 2)
